...
/* DHT - mede temperatura e Umidade  */
DHT dht(DHTPIN, DHTTYPE);

//Definicão EEPROM 
int currentAddress = 10; 

//Definicão BME, Endereço I2C padrão do sensor
#define BME280_ADDR 0x76

// Registrador de soft-reset do BME280 (era indefinido/inconsistente no original)
#define BME280_REG_RESET 0xE0

//Registrados para armazenar as calibrações da fábrica de Bosch 
struct BME280_Calib {
...
//Variaveis Globais de Controle e Armazenamento 
static BME280_Calib calib; 
static int32_t t_fine = 0; //Fator de correção cruzada vital exigido pelo Bosch 
...
bool dhtUpdate(float &temperatura, float &umidade) {
    ...
    //se o valor retornado em um numero colocar o valor da umidade e temperatura
    if (!isnan(h) && !isnan(t)) {
        umidade = h;
        temperatura = t;
        return true;
            ...
            { 
                ...
                { 
                    ...
                    { 
                        ...
                        Wire.write(0x6B); //Registro PWR_MGMT _1
                        Wire.write(0); //zero o modo sleep
...
//Inicializa o Hardware, lê o ID e baixa as tabelas de calibracao do CHIP 
bool bme_init() { 
    uint8_t id = 0; 
    bme_read_regs(0xD0, &id, 1);
    if (id != 0x60 && id != 0x58) return false; //Valida se é BME280 (0x60) ou BMP280 (0x58)

    //soft reset: garante um estado limpo do chip antes de ler a calibracao 
    bme_write_reg(BME280_REG_RESET, 0xB6); 
    delay(5); //tempo de start up apos reset 
    uint32_t t0 = millis(); 
    while(bme_read_reg8(BME280_REG_RESET) & 0x01) { 
        if(millis() - t0 > 100) break; //timeout de 100ms
    }
...
void eepromWriteByte(uint16_t address, uint8_t* data, uint16_t length) {
    while(length > 0) { 
// sensores.cpp — in eepromWriteByte()
        ...
        delay(5); // Aguarda a escrita na EEPROM
